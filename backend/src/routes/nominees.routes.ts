...
router.patch('/:year/:categoryId/:nomineeId', authenticate, requireSuperuser, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const categoryIdWithYear = categoryId.endsWith(`-${year}`) ? categoryId : `${categoryId}-${year}`;
    ...
    const fields = ['blurb_sentence_1', 'blurb_sentence_2', 'imdb_url', 'letterboxd_url'] as const;
