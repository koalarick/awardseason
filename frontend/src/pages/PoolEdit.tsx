...
function getNomineeInfoFromData(nominee: Nominee, category: Category): {
  category: string
  nominee: string
...
function filterOddsHistoryByDay(
  ...
  if (filteredHistory.length > 0 && filteredHistory[filteredHistory.length - 1].snapshotTime !== history[history.length - 1].snapshotTime) {
...
function getLightestColor(imageColors: { primary: string; secondary: string; accent: string }): string {
  const colors = [
...
function extractImageColors(imageSrc: string): Promise<{ primary: string; secondary: string; accent: string; averageBrightness: number }> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        const colors: Array<{ r: number; g: number; b: number; brightness: number; saturation: number; vibrancy: number }> = []
        ...
        for (const point of samplePoints) {
          ...
          const vibrancy = saturation * (brightness > 50 && brightness < 200 ? 1.2 : 1) * (brightness / 255)
        ...
        const secondary = vibrantColors[Math.min(1, vibrantColors.length - 1)] || colors[Math.floor(colors.length / 2)]
        const accent = vibrantColors.slice(0, 3).sort((a, b) => b.brightness - a.brightness)[0] || vibrantColors[0] || colors[0]
...
export default function PoolEdit() {
  ...
  const [imageColors, setImageColors] = useState<{ primary: string; secondary: string; accent: string; averageBrightness: number } | null>(null)
  const [nomineeImageColors, setNomineeImageColors] = useState<Map<string, { primary: string; secondary: string; accent: string; averageBrightness: number }>>(new Map())
  ...
  const { data: predictions } = useQuery({
    ...
    queryFn: async () => {
      if (isViewingOtherSubmission && targetUserId) {
        ...
        return allPredictions.filter((p: any) => p.userId === targetUserId || p.user?.id === targetUserId)
      } else {
  ...
  const { data: oddsHistory } = useQuery({
    queryKey: ['oddsHistory', selectedNomineeInfo?.category.id, selectedNomineeInfo?.nominee.id, pool?.year],
    queryFn: async () => {
      if (!selectedNomineeInfo || !pool?.year) return null
      const fullCategoryId = `${selectedNomineeInfo.category.id}-${pool.year}`
      try {
        const response = await api.get(`/odds/${fullCategoryId}/${selectedNomineeInfo.nominee.id}/history`)
      ...
      const oddsPromises = categories.map(async (category) => {
        ...
        try {
          ...
          return {
            ...
            nominees: (response.data.nominees || []) as Array<{ nomineeId: string; odds: number | null }>
          }
        } catch (error: any) {
  ...
  const handleNomineeSelect = (categoryId: string, nomineeId: string) => {
    ...
    if (existingPrediction && existingPrediction.nomineeId !== nomineeId) {
      ...
      if (oldOdds !== null && oldOdds !== undefined) {
        ...
        const currentOddsForOldNominee = categoryOddsForThis.find((o: any) => o.nomineeId === existingPrediction.nomineeId)?.odds || null
        
        // Only show warning if odds have changed for the current selection
        if (currentOddsForOldNominee !== null && currentOddsForOldNominee !== undefined && 
            Math.abs(currentOddsForOldNominee - oldOdds) > 0.01) {
          const category = categories?.find((c: Category) => c.id === categoryId)
          const oldNominee = category?.nominees.find((n: Nominee) => n.id === existingPrediction.nomineeId)
          const newNominee = category?.nominees.find((n: Nominee) => n.id === nomineeId)
          const currentOddsForNewNominee = categoryOddsForThis.find((o: any) => o.nomineeId === nomineeId)?.odds || null
          ...
          {categories && predictions && (() => {
            ...
            predictions.forEach((prediction: any) => {
              ...
              if (category) {
                ...
                const scoring = calculateScoringPreview(nomineeOdds, basePoints, multiplierEnabled, multiplierFormula)
                totalPossiblePoints += scoring.totalPoints
                
                // Check if this prediction is correct (matches actual winner)
                if (actualWinners) {
                  const winner = actualWinners.find((w: any) => w.categoryId === prediction.categoryId)
                  if (winner) {
            ...
            const percentCorrect = totalWithWinners > 0 ? Math.round((correctCount / totalWithWinners) * 100) : 0
            
            return (
              <div className="px-4 sm:px-6 py-3 border-b border-gray-200">
                <div className={`grid ${hasWinners ? 'grid-cols-4' : 'grid-cols-3'} gap-2 md:gap-4`}>
                  ...
                  <div className="flex flex-col items-center text-center">
                    <span className="text-xs text-gray-500 uppercase tracking-wide">Possible</span>
                    <span className="font-semibold text-sm oscars-gold">{totalPossiblePoints.toFixed(1)}</span>
                  </div>
                  <div className="flex flex-col items-center text-center">
            ...
            return (
              <div className="border-t border-gray-200 bg-gray-50">
                <div className="px-4 sm:px-6 py-3">
                  <h3 className="oscars-font text-sm font-semibold oscars-dark mb-2">How It Works</h3>
                  <div className="relative">
                    <div className={`text-xs text-gray-600 space-y-3 ${!isHowItWorksOpen ? 'line-clamp-2' : ''}`}>
                      <p>
                        Pick who you think is going to win in each category. You can update your predictions as many times as you want until 24 hours before the ceremony.
                      </p>
                      <div>
                        <p className="font-medium text-gray-700 mb-1">Scoring</p>
                        <p>
                          Each category is assigned a certain point value. You earn those points for each correct prediction
                          {multiplierEnabled && (
                            <span>. When the multiplier is enabled, picking underdogs earns you more points.</span>
                          )}
                        </p>
                      </div>
                      {multiplierEnabled && (
                        <div>
                          ...
                          <ul className="space-y-1.5 list-disc list-inside">
                            <li>
                              <strong className="text-gray-700">Odds are saved when you select:</strong> The current odds percentage is saved with your pick and used to calculate your potential points.
                            </li>
                            <li>
                              <strong className="text-gray-700">Odds change over time:</strong> Betting odds update regularly as the ceremony approaches. Your scoring automatically uses the best odds available.
                            </li>
                            <li>
                              <strong className="text-gray-700">Lower odds mean more points:</strong> Lower percentage odds (worse chances) result in a higher multiplier and more points if you're correct.
                            </li>
                          </ul>
                        </div>
                      )}
        ...
        {categories && predictions && showStickySummary && (
          <div ref={stickySummaryRef} className="sticky bg-white border-b border-gray-200 z-30 py-3" style={{ top: `${headerHeight}px` }}>
            <div className="flex items-center justify-between px-4 md:px-6 gap-3">
              <span className="oscars-font font-bold oscars-dark text-sm md:text-base truncate flex-shrink min-w-0">{submissionName}</span>
                ...
                <div className="hidden md:flex items-center gap-4 text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`font-semibold ${
                      predictions.length === categories.length ? 'text-green-600' : 'text-yellow-600'
                    }`}>
                      {predictions.length}/{categories.length}
                    </span>
                  </div>
                    ...
                    return (
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="oscars-gold font-bold">{totalPossiblePoints.toFixed(1)}</span>
                        </div>
                        <div className="flex items-center gap-2">
            ...
            {isViewingOtherSubmission ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {categoryGroups.flatMap((group) =>
                      group.categoryIds
                        ...
                        .filter((item): item is { category: any; prediction: any } => item !== null)
                    ).map(({ category, prediction }) => {
                    
                    const categoryPoints = (poolSettings?.categoryPoints as Record<string, number>) || {}
                    const basePoints = categoryPoints[category.id] || category.defaultPoints || 0
                    const multiplierEnabled = poolSettings?.oddsMultiplierEnabled ?? true
                    const multiplierFormula = poolSettings?.oddsMultiplierFormula || 'log'
                    const categoryOddsForThis = getCategoryOdds(category.id)
                    const selectedNominee = category.nominees.find((n: Nominee) => n.id === prediction.nomineeId)
                    
                    if (!selectedNominee) return null
                    
                    const currentOdds = categoryOddsForThis.find((o: any) => o.nomineeId === selectedNominee.id)?.odds || null
                    const oddsForScoring = prediction?.oddsPercentage !== null && prediction?.oddsPercentage !== undefined
                      ? prediction.oddsPercentage
                      : currentOdds
                    const scoring = calculateScoringPreview(oddsForScoring, basePoints, multiplierEnabled, multiplierFormula)
                    ...
                    const actualWinnerNominee = winner ? category.nominees.find((n: Nominee) => n.id === winner.nomineeId) : null
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          >
                            <img
                              ...
                              onLoad={(e) => {
                                ...
                                if (!nomineeImageColors.has(colorKey)) {
                                  extractImageColors(imgSrc)
                                    .then(colors => {
                                      setNomineeImageColors(prev => new Map(prev).set(colorKey, colors))
                                    })
                                    .catch(() => {
                              ...
                              onError={(e) => {
                                console.error(`Failed to load image: ${getNomineeImage(selectedNominee, category.id, pool.year)}`)
                          ...
                          <div className="flex-1 min-w-0">
                            {selectedNominee.film && selectedNominee.name !== selectedNominee.film && (
                              <>
                                <h5 className={`font-bold text-sm truncate ${
                                  hasWinner && !isCorrect ? 'line-through text-gray-500' : 'oscars-dark'
                                }`}>
                                  {selectedNominee.name}
                                </h5>
                                <h5 className={`font-normal text-xs italic truncate mt-0.5 ${
                                  hasWinner && !isCorrect ? 'line-through text-gray-400' : 'text-gray-600'
                                }`}>
                            ...
                            {!oddsLoading && (
                              <div className="text-xs text-gray-600 mt-0.5">
                                {(prediction?.originalOddsPercentage !== null && prediction?.originalOddsPercentage !== undefined && prediction?.nomineeId === selectedNominee.id) ? (
                                  <>
                                    {currentOdds !== null && currentOdds > 0 && currentOdds !== prediction.originalOddsPercentage ? (
                                      <div className="text-gray-700">
                                        Chance to win: <span className="font-semibold">{currentOdds.toFixed(0)}%</span> <span className="text-gray-500">(Selected at: {prediction.originalOddsPercentage.toFixed(0)}%)</span>
                                      </div>
                                    ) : (
                                      <span>Selected at: <span className="font-semibold">{prediction.originalOddsPercentage.toFixed(0)}%</span></span>
                                    )}
                                  </>
                                ) : prediction?.oddsPercentage !== null && prediction?.oddsPercentage !== undefined ? (
                                  currentOdds !== null && currentOdds > 0 ? (
                                    <span>Scoring at: <span className="font-semibold text-yellow-700">{currentOdds.toFixed(0)}%</span></span>
                                  ) : (
                                    <span>Selected at: <span className="font-semibold">{prediction.oddsPercentage.toFixed(0)}%</span></span>
                                  )
                                ) : currentOdds !== null && currentOdds > 0 ? (
                                  <span>Chance to win: <span className="font-semibold">{currentOdds.toFixed(0)}%</span></span>
                                ) : null}
                              </div>
                            )}
                            {hasWinner && !isCorrect && actualWinnerNominee && (
                              <div className="mt-2 pt-2 border-t border-red-300">
                                <div className="text-xs text-red-700 font-semibold mb-1">Actual Winner:</div>
                                <div className="flex items-center gap-2">
                                  <div className="w-8 h-8 flex-shrink-0 rounded overflow-hidden flex items-center justify-center bg-white">
                                    <img
                                      src={getNomineeImage(actualWinnerNominee, category.id, pool.year)}
                                  ...
                                  <div>
                                    {actualWinnerNominee.film && actualWinnerNominee.name !== actualWinnerNominee.film && (
                                      <>
                          ...
                          {multiplierEnabled && (
                            <div className="flex flex-col items-end text-sm flex-shrink-0">
                              ...
                              {hasWinner ? (
                                isCorrect ? (
                                  <>
                                    <span className="text-xs text-green-700 font-semibold mb-0.5">Earned</span>
                                  ...
                                  <>
                                    <span className="font-bold text-red-700 text-base mb-0.5">0 pts</span>
              ...
              <>
                ...
                <div 
                  ...
                  style={{ 
                    top: categories && predictions && showStickySummary 
                    ...
                    <div className="space-y-4">
                        {categoryGroups
                          ...
                          ?.categoryIds.map((categoryId) => {
                            ...
                            const hasWinner = !!winner
                            const isCorrect = hasWinner && prediction && winner.nomineeId === prediction.nomineeId
                              ...
                              <div key={category.id} className={`${
                                ...
                                >
                                  <h4 className="oscars-font text-base font-bold oscars-dark">
                                    ...
                                    {hasWinner && (
                                      <span className={`ml-2 text-sm font-semibold ${
                                        isCorrect ? 'text-green-700' : prediction ? 'text-red-700' : ''
                                      }`}>
                                  ...
                                  <span className="text-sm font-bold oscars-dark pr-2 md:pr-2.5">
                                    {multiplierEnabled ? 'Base: ' : ''}{basePoints} pts
                                  </span>
                                </div>
                                <div className="space-y-2 md:grid md:grid-cols-5 md:gap-2 md:space-y-0">
                                  {[...category.nominees]
                                    .sort((a, b) => {
                                      const oddsA = categoryOddsForThis.find((o: any) => o.nomineeId === a.id)?.odds || 0
                                      const oddsB = categoryOddsForThis.find((o: any) => o.nomineeId === b.id)?.odds || 0
                                      if (oddsB !== oddsA) {
                                        return oddsB - oddsA
                                      }
                                    ...
                                    .map((nominee) => {
                                      ...
                                      const currentOdds = categoryOddsForThis.find((o: any) => o.nomineeId === nominee.id)?.odds || null
                                      ...
                                      if (isSelected && prediction?.originalOddsPercentage !== null && prediction?.originalOddsPercentage !== undefined && prediction?.nomineeId === nominee.id) {
                                        if (currentOdds !== null && currentOdds !== undefined) {
                                          // Use min of current and original - lower percentage = better bonus
                                          oddsForScoring = Math.min(currentOdds, prediction.originalOddsPercentage)
                                        } else {
                                          // Fallback to stored odds if current not available
                                          oddsForScoring = prediction.oddsPercentage ?? prediction.originalOddsPercentage
                                        }
                                      } else if (currentOdds !== null && currentOdds !== undefined) {
                                        oddsForScoring = currentOdds
                                      } else if (isSelected && prediction?.oddsPercentage !== null && prediction?.oddsPercentage !== undefined) {
                                        oddsForScoring = prediction.oddsPercentage
                                      }
                                      const scoring = oddsForScoring !== null
                                        ? calculateScoringPreview(oddsForScoring, basePoints, multiplierEnabled, multiplierFormula)
                                      ...
                                      const isIncorrect = hasWinner && isSelected && winner.nomineeId !== nominee.id
                                      const isActualWinnerButNotSelected = hasWinner && isCorrect && !isSelected
                                      ...
                                      return (
                                        <div
                                          ...
                                          onClick={(e) => {
                                            ...
                                            if ((e.target as HTMLElement).closest('.nominee-image-container')) {
                                              return
                                            }
                                            if (canSelect) {
                                        ...
                                        >
                                          ...
                                          >
                                            <img
                                              ...
                                              alt={nominee.name}
                                              className="w-full h-full object-contain"
                                              onClick={(e) => handleNomineeImageClick(e, nominee, category)}
                                              onLoad={(e) => {
                                                const imgSrc = e.currentTarget.src
                                              ...
                                              onError={(e) => {
                                                console.error(`Failed to load image: ${getNomineeImage(nominee, category.id, pool.year)}`)
                                                e.currentTarget.style.display = 'none'
                                              }}
                                            />
                                            {/* Info icon badge - mobile only, subtle */}
                                                        ...
                                                        {currentOdds !== null && currentOdds > 0 && currentOdds !== prediction.originalOddsPercentage ? (
                                                          <div className="text-gray-700">
                                                            <div className="flex flex-wrap items-baseline gap-1">
                                                              <span>Win: <span className="font-semibold">{currentOdds.toFixed(0)}%</span></span>
                                                              <span className="text-gray-500">(was {prediction.originalOddsPercentage.toFixed(0)}%)</span>
                                                              <span className={`${currentOdds < prediction.originalOddsPercentage ? 'text-yellow-600' : 'text-green-600'}`}>
                                                                {currentOdds < prediction.originalOddsPercentage ? 'Odds updated' : 'Odds locked'}
                                                              </span>
                                                            </div>
                                                          </div>
                                                        ) : (
                                                          <span>Win: <span className="font-semibold text-yellow-700">{prediction.originalOddsPercentage.toFixed(0)}%</span></span>
                                                        )}
                                                      </>
                                                    ) : (
                                                      currentOdds !== null && currentOdds > 0 ? (
                                                        <span>Scoring: <span className="font-semibold text-yellow-700">{currentOdds.toFixed(0)}%</span></span>
                                                      ) : (
                                                        prediction?.oddsPercentage !== null && prediction?.oddsPercentage !== undefined && (
                                                          <span>Scoring: <span className="font-semibold text-yellow-700">{prediction.oddsPercentage.toFixed(0)}%</span></span>
                                                        )
                                                      )
                                                    )}
                                                  </>
                                              ...
                                              <div className="mt-3 pt-3 border-t border-gray-200">
                                                <div className="flex items-center justify-between">
                                                  ...
                                                  <div className="flex items-baseline gap-2">
                                                    ...
                                                    {hasWinner && isIncorrect && (
                                                      <>
                                                        <span className="font-bold text-red-700 text-base">0</span>
                                                        <span className="text-gray-400 line-through text-xs">
                                                          {scoring.totalPoints.toFixed(1)}
                                                        </span>
                                                      </>
                                          ...
                                          <div className="hidden md:flex md:flex-col md:w-full md:gap-0">
                                              ...
                                              <div className="text-xs text-gray-600 leading-tight mt-1">
                                                {isSelected ? (
                                                    ...
                                                    {prediction?.originalOddsPercentage !== null && 
                                                     ...
                                                     prediction?.nomineeId === nominee.id ? (
                                                        ...
                                                        {currentOdds !== null && currentOdds > 0 && currentOdds !== prediction.originalOddsPercentage ? (
                                                          <div className="text-gray-700">
                                                            <span className="inline-block w-full">
                                                              Chance to win: <span className="font-semibold">{currentOdds.toFixed(0)}%</span> <span className="text-gray-500">(was {prediction.originalOddsPercentage.toFixed(0)}%)</span>
                                                            </span>
                                                          </div>
                                                        ) : (
                                                          <div className="text-gray-700">
                                                            <span className="inline-block w-full">
                                                              Chance to win: <span className="font-semibold text-yellow-700">{prediction.originalOddsPercentage.toFixed(0)}%</span>
                                                              <span className="text-gray-500 invisible"> (was 0%)</span>
                                                            </span>
                                                          </div>
                                                        )}
                                                      </>
                                                      ...
                                                      currentOdds !== null && currentOdds > 0 ? (
                                                        ...
                                                        prediction?.oddsPercentage !== null && prediction?.oddsPercentage !== undefined && (
                                                          <span>Scoring at: <span className="font-semibold text-yellow-700">{prediction.oddsPercentage.toFixed(0)}%</span></span>
                                                        )
                                                      )
                                                    )}
                                                  </>
                                                  ...
                                                  currentOdds !== null && currentOdds > 0 && (
                                                    <div className="text-gray-700">
                                                      <span className="inline-block w-full">
                                                        Chance to win: <span className="font-semibold">{currentOdds.toFixed(0)}%</span>
                                                        <span className="text-gray-500 invisible"> (was 0%)</span>
                                                      </span>
                                                    </div>
                                                  )
                                            ...
                                            {multiplierEnabled && (
                                              <div className="mt-2 pt-2 border-t border-gray-200">
                                                ...
                                                {isSelected && prediction?.originalOddsPercentage !== null && 
                                                 prediction?.originalOddsPercentage !== undefined &&
                                                 prediction?.nomineeId === nominee.id &&
                                                 currentOdds !== null && currentOdds > 0 && 
      ...
      {pendingSelection && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md mx-4">
            <h3 className="oscars-font text-xl font-bold oscars-dark mb-4">Change Your Selection?</h3>
            <p className="text-gray-700 mb-3">
              <strong>{pendingSelection.categoryName}:</strong> You're about to change from <strong className="text-yellow-700">{pendingSelection.oldNomineeName}</strong> (selected at {pendingSelection.oldOdds?.toFixed(0)}%) to <strong className="text-yellow-700">{pendingSelection.nomineeName}</strong>{pendingSelection.currentOdds !== null && pendingSelection.currentOdds !== undefined ? ` (current odds: ${pendingSelection.currentOdds.toFixed(0)}%)` : ''}.
            </p>
            <p className="text-gray-700 mb-6">
              <strong className="text-red-600">Important:</strong> If you change your selection and then switch back to <strong>{pendingSelection.oldNomineeName}</strong> later, you'll get the <strong>current odds at that time</strong>, not your original {pendingSelection.oldOdds?.toFixed(0)}% odds. Your original odds will be lost.
            </p>
        ...
        >
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between gap-4 min-w-0" data-modal-header={selectedNomineeInfo.nominee.id.replace(/[^a-zA-Z0-9]/g, '-')}>
                <h2 
                  id="modal-title" 
            ...
            >
              ...
              >
                ...
                {imageError ? (
                  ...
                  <>
                    <img
                      src={getNomineeImage(selectedNomineeInfo.nominee, selectedNomineeInfo.category.id, pool.year)}
                    ...
                    <p 
                      ...
                      style={imageColors ? (() => {
                        ...
                        return {
                          ...
                          textShadow: isDarkMode ? `0 1px 1px ${hexToRgba(imageColors.secondary, 0.2)}` : 'none'
                        }
                      })() : { color: '#374151' }}
                    >
                  ...
                  {oddsHistory && oddsHistory.length > 0 ? (
                    <div className="flex flex-col flex-1 min-h-0 -mx-4 md:-mx-6">
                      <div className="p-2 sm:p-3 flex flex-col flex-1 min-h-0">
                        {(() => {
                          const history = oddsHistory as Array<{ oddsPercentage: number; snapshotTime: string }>
                          const filteredHistory = filterOddsHistoryByDay(history)
                          
                          const firstOdds = filteredHistory[0].oddsPercentage
                          const currentOdds = filteredHistory[filteredHistory.length - 1].oddsPercentage
                              ...
                              <div className="flex justify-between items-start mb-2 flex-shrink-0">
                                <div className="flex flex-col">
                                  <span 
                                    ...
                                    style={imageColors ? (() => {
                                      const isDarkMode = imageColors.averageBrightness < 140
                                      return { color: getTextColorForMode(imageColors.primary, isDarkMode) }
                                    })() : { color: '#6b7280' }}
                                  >
                                    Opening
                                  </span>
                                  <span 
                                    ...
                                    style={oddsTextColor && imageColors ? { 
                                      color: getTextColorForMode(oddsTextColor, imageColors.averageBrightness < 140),
                                      fontWeight: '700',
                                      letterSpacing: '0.01em'
                                    } : oddsTextColor ? {
                                      color: oddsTextColor,
                              ...
                              <div className="flex-1 min-h-0 w-full mt-2">
                                ...
                                <defs>
                                  <linearGradient id={gradientId} x1="0%" y1="0%" x2="0%" y2="100%">
                                    <stop offset="0%" stopColor={gradientColor1} stopOpacity={imageColors ? "0.6" : "0.2"} />
                                    <stop offset="50%" stopColor={gradientColor2} stopOpacity={imageColors ? "0.4" : "0.1"} />
                                    <stop offset="100%" stopColor={gradientColor2} stopOpacity={imageColors ? "0.15" : "0.05"} />
                                  </linearGradient>
                                </defs>
                                {/* Area fill */}
                                <path
                ...
                <div className="hidden lg:flex flex-col lg:flex-row gap-4 md:gap-6 lg:gap-6 flex-1 min-h-0">
                  ...
                  <div className="flex-1 min-w-0 lg:w-3/5 flex flex-col min-h-0">
                          ...
                          {(() => {
                            ...
                            return (
                              ...
                              >
                                ...
                                {/* Chart */}
                                <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none" style={{ marginTop: '40px', height: 'calc(100% - 40px)' }}>
                                <defs>
                                  <linearGradient id={gradientId} x1="0%" y1="0%" x2="0%" y2="100%">
                                    <stop offset="0%" stopColor={gradientColor1} stopOpacity={imageColors ? "0.7" : "0.2"} />
                                    <stop offset="50%" stopColor={gradientColor2} stopOpacity={imageColors ? "0.5" : "0.1"} />
                                    <stop offset="100%" stopColor={gradientColor2} stopOpacity={imageColors ? "0.3" : "0.05"} />
                                  </linearGradient>
                                </defs>
                                  {/* Area fill */}
                                  <path
                    ...
                    <div className="hidden lg:flex flex-col">
                      <div className="p-3 lg:p-5 flex flex-col border-t"
                        style={imageColors ? (() => {
                          const isDarkMode = imageColors.averageBrightness < 140
                          return {
                            background: 'transparent',
                            borderTopColor: isDarkMode ? hexToRgba(imageColors.primary, 0.3) : '#e5e7eb',
                            borderTopWidth: '1px'
                          }
                        })() : {
                          background: 'transparent',
                      ...
                      {!isViewingOtherSubmission && (() => {
                        const existingPrediction = getPredictionForCategory(selectedNomineeInfo.category.id)
                        const isSelected = existingPrediction?.nomineeId === selectedNomineeInfo.nominee.id
                        
                        return (
                          <button
                            onClick={() => {
                              handleNomineeSelect(selectedNomineeInfo.category.id, selectedNomineeInfo.nominee.id)
                              closeNomineeModal()
                            }}
              ...
              <>
                ...
                <div className="space-y-2 mb-4">
                  {filteredSubmissions.map((submission: any) => (
                    ...
                    >
                      <div className="font-semibold text-gray-900 text-sm">{submission.poolName}</div>
                    </button>
                  ))}
                </div>
                ...
                {copyResult && (
                  <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded text-sm text-green-800">
                    Copied {copyResult.copied} {copyResult.copied === 1 ? 'selection' : 'selections'}
                    {copyResult.skipped > 0 && (
                      <span className="block mt-1 text-xs">
                        Skipped {copyResult.skipped} {copyResult.skipped === 1 ? 'category' : 'categories'} with winners.
                      </span>
                    )}
                  </div>
                )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md mx-4">
            ...
            <p className="text-gray-700 mb-6">
              ...
              {announcedCount > 0 && (
                <>
                  <strong className="text-yellow-600">Note:</strong> Categories with announced winners ({announcedCount} {announcedCount === 1 ? 'category' : 'categories'}) will not be cleared.
                  <br /><br />
                </>
              )}
              <strong className="text-red-600">Important:</strong> When you make new picks, they will use the current odds, which may be different from when you originally selected them. This means your potential points may change.
            </p>
            {skippedCount !== null && skippedCount > 0 && (
              <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded">
                <p className="text-sm text-yellow-800">
                  <strong>Skipped {skippedCount} {skippedCount === 1 ? 'category' : 'categories'} with announced winners.</strong>
                </p>
              </div>
            )}
            <div className="flex gap-3 justify-end">
